...
const configuracionPorUbigeo = {
    "080108": {
      ...
      ortofotos: "ortofoto",
      // Niveles de zoom permitidos. El mapa NO podrá alejarse por debajo
      // de zoom_minimo (se bloquea) y solo permitirá acercarse hasta zoom_maximo.
      zoom_minimo: 14,
      zoom_maximo: 21,
      ...
      proyeccion_display_texto: "WGS84 - Zona 19",
    },
    "080601": {
    ...
    "081304": {
      HOST: IS_DEV ? "http://127.0.0.2" : "http://192.168.50.7",
      PORT: IS_DEV ? "80" : "80",
      xy: [-8073322.4, -1477490.94],
      zoom: 18,
      ...
      proyeccion_display_texto: "WGS84 - Zona 18",
    },
  },
  configuracionUbigeo =
...
// Construye el HTML de una leyenda (título + un item por campo con su color).
// Es la única plantilla de leyenda; servicios básicos, clasificación y tipo de
// persona la reutilizan pasando su propio título y lista de campos.
...
export const ubigeo = UBIGEO,
  ...
  ortoFotos = ortofotos,
  zoomMapa = zoom,
  // Niveles de zoom límite del mapa principal. zoomMinimo bloquea el alejamiento
  // por debajo del zoom inicial; zoomMaximo limita el acercamiento.
  zoomMinimo = zoom_minimo ?? zoom,
  zoomMaximo = zoom_maximo ?? 21,
  ...
  direccionServicioMapCache = IS_WIN //Windows http://[url]:[port]/mapcache? | Linux http://[url]:[port]/mapcache?
    ? `${HOST}:${PORT}/mapcache/?`
    : `${HOST}:${PORT}/cgi-bin/mapcache/?`,
  direccionApiGIS = IS_WIN ? `${HOST}:5000/` : `${HOST}:${PORT}/api-gis/`,
...
// Capas base disponibles. La capa por defecto es Open Street Map. Incluye las
// capas de Google (satélite y calles) además de la ortofoto del municipio y
// OSM Noche.
export const CAPAS_BASE = [
  { id: "osm", titulo: "Open Street Map", checked: true },
  { id: "ortofoto", titulo: "Ortofoto", checked: false },
  { id: "esriModoNoche", titulo: "OSM Noche", checked: false },
  { id: "googleMapSatelite", titulo: "Google Satélite", checked: false },
