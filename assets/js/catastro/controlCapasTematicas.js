...
// Alerta Bootstrap descartable para los mensajes de búsqueda (lote / vía).
// Centraliza el marcado que antes se repetía en cada validación/resultado.
function alertaBusqueda(tipo, mensaje) {
  return `<div class="alert alert-sm alert-${tipo} alert-dismissible fade show" role="alert"><strong>Error!</strong> ${mensaje}<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Cerrar"></button></div>`;
}

function buscarEnCapaWFS(tipoCapa, campo, valorBusqueda) {
  // Acepta tanto (tipoCapa, campo, valorBusqueda) como un único objeto
  // { tipoCapa, campo, valorBusqueda }; se normaliza una sola vez aquí.
  ...
  return ejecutarConsultaWFS(parametrosBusqueda).then(
...
// Lanza la consulta WFS GetFeature con un filtro LIKE. Recibe los parámetros ya
// normalizados (no vuelve a normalizar).
function ejecutarConsultaWFS(parametrosBusqueda) {
...
function renderizarSidebarDinamico(gruposWms) {
  ...
  // La selección de capa base se gestiona desde el panel "Capa base" de la
  // barra derecha (controlCapaBase.js); por eso ya no se lista aquí.
  const bloques = [
...
const ejecutarBusquedaPorIdLote = () => {
  ...
  if (!valorBusqueda.length) {
    mensajeBuscarLote.innerHTML = alertaBusqueda("warning", "Ingresar un campo.");
  ...
  buscarEnCapaWFS({
    ...
    .then(({ data, totalRegistros }) => {
      if (totalRegistros > 0) {
        ...
        ocultarListadoLotes();
        ocultarListadoVias();
        mensajeBuscarLote.innerHTML = alertaBusqueda(
          "danger",
...
buscarViaHab?.addEventListener("click", function () {
  ...
  if (!valorBusqueda.length) {
    mensajeBuscarViaHab.innerHTML = alertaBusqueda(
      "warning",
      "Ingresar un campo.",
    );
