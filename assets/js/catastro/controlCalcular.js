...
if (btnCalcularArea) {
  btnCalcularArea.addEventListener('click', function () {
    ...
    if (!activo) {
      ...
      iniciarMedicion('Polygon', formatearArea);
    }
  });
}

if (btnCalcularPerimetro) {
  btnCalcularPerimetro.addEventListener('click', function () {
    const activo = this.classList.contains('active');
    limpiarTodo();
    if (!activo) {
      this.classList.add('active');
      iniciarMedicion('LineString', formatearLongitud);
    }
  });
}
